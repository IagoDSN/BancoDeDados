...
INSERT INTO `professores` (`codProfessor`, `nomeProfessor`, `formacaoProfessor`) VALUES
	(1, 'Cristina Almeida', 'Ciência da Computação'),
	(3, 'João Roberto', NULL),
	(4, 'Vlander Verdades', NULL),
	(5, 'Marcelo Pereira', 'Educação Física'),
	(6, 'Daniela Guimarães', 'Ciência da computação'),
	(7, 'Poliana Ester da Silva', NULL);
sys
-- Copiando estrutura para tabela aula2e.professoresporturma
DROP TABLE IF EXISTS `professoresporturma`;
CREATE TABLE IF NOT EXISTS `professoresporturma` (
...
-- Copiando dados para a tabela aula2e.professoresporturma: ~0 rows (aproximadamente)
...
-- Copiando dados para a tabela aula2e.turmas: ~6 rows (aproximadamente)
