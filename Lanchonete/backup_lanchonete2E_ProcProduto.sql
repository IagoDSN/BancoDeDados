...
-- Copiando dados para a tabela lanchonete2e2025.marca: ~8 rows (aproximadamente)
...
CREATE PROCEDURE `proc_ajustesPrecoProdutos`(
	IN `opcao` VARCHAR(150),
	IN `margemDesejada` DECIMAL(4,1)
)
BEGIN
	SELECT * FROM produto;
   IF opcao = 'diminuir' THEN
      UPDATE produto
      SET precoVenda = precoVenda - (precoVenda * margemDesejada/100), margemLucro = margemDesejada;
      SELECT 'Atualizado com sucesso';
   ELSEIF opcao = 'aumentar' THEN
      UPDATE produto
      SET precoVenda =  precoVenda + (precoVenda * margemDesejada/100), margemLucro = margemDesejada;
      SELECT 'Atualizado com sucesso';
   ELSE
      SELECT 'Opção invalida' AS ERRO;
   END IF;
	SELECT * FROM produto;
END//
DELIMITER ;
...
CREATE PROCEDURE `proc_alteraProduto`(
    ...
    COMMENT 'Altera campos obrigatórios e opicionais do produto.'
BEGIN
	...
	if (@contador = 1)
		then UPDATE produto SET nomeProduto = nomeAlterar, quantEstoque = quantEstoqueAlterar,
 		quantMinima = quantMinimaAlterar, precoCusto = precoCustoAlterar, precoVenda = precoVendaAlterar,
  		margemLucro = margemLucroAlterar, validade = validadeAlterar, marca_codMarca = qualMarca,
...
-- Copiando estrutura para procedure lanchonete2e2025.proc_RelatorioProduto
...
BEGIN
    if (LOWER(tipoRelatorio) = "simples")
        then SELECT p.nomeProduto, p.precoVenda from produto AS p;
        ELSE if (LOWER(tipoRelatorio) = "personalizado")
            then SELECT p.nomeProduto, p.precoVenda, c.nomeCategoria, m.nomeMarca FROM produto AS p
                INNER JOIN categoria AS c ON p.categoria_codCategoria = c.codCategoria
                INNER JOIN marca AS m ON p.marca_codMarca = m.codMarca;
            ELSE if (LOWER(tipoRelatorio) = "estoque")
                then SELECT p.codProduto, p.nomeProduto, p.quantEstoque, p.quantMinima FROM produto AS p;
                ELSE if (LOWER(tipoRelatorio) = "abaixoestoque")
                    then SELECT p.nomeProduto FROM produto AS p WHERE p.quantEstoque < p.quantMinima;
                    ELSE if (LOWER(tipoRelatorio) = "vigilanciasanitaria")
                        then SELECT p.codProduto, p.nomeProduto, DATE_FORMAT(p.validade, "%d/%m/%Y") AS validade FROM produto AS p;
                        ELSE if (LOWER(tipoRelatorio) = "total")
                        then SELECT COUNT(*) FROM produto;
                        ELSE SIGNAL SQLSTATE "45000" SET MESSAGE_TEXT = "Opção inválida!";
                        END if;
                    END if;
                END if;
            END if;
...
CREATE PROCEDURE `proc_relatoriosCategoria`(
	IN `numeroRelatorio` VARCHAR(50)
)
BEGIN
	if (numeroRelatorio = 1)
		then SELECT * FROM categoria;
		ELSE if (numeroRelatorio = 2)
					then SELECT COUNT(*) AS totalCategorias FROM categoria;
					ELSE if (numeroRelatorio = 3)
							then SELECT nomeCategoria FROM categoria ORDER BY nomeCategoria DESC;
							ELSE if (numeroRelatorio = 4)
									then SELECT nomeCategoria FROM categoria ORDER BY nomeCategoria ASC;
	...
	if (tipoRelatorio = "C")
		...
		ELSE if (tipoRelatorio = "D")
					...
					ELSE if (tipoRelatorio = "T")
								...
								ELSE SELECT "Tipo inexistente de relatório" AS ERRO;
							END if;
				END if;
	END if;
END//
...
-- Copiando dados para a tabela lanchonete2e2025.produto: ~1 rows (aproximadamente)
INSERT INTO `produto` (`codProduto`, `nomeProduto`, `quantEstoque`, `quantMinima`, `precoCusto`, `precoVenda`, `margemLucro`, `validade`, `marca_codMarca`, `categoria_codCategoria`) VALUES
	(1, 'coxinha de costela', 20, 10, 5.00, 6.00, 0.0, NULL, 3, 1),
	(2, 'Prestigio', 7, 35, 2.50, 2.25, 0.0, '2025-11-15', 7, 5);
