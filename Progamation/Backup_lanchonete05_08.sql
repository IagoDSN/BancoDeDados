...
-- Copiando dados para a tabela lanchonete2e2025.categoria: ~4 rows (aproximadamente)
...
-- Copiando dados para a tabela lanchonete2e2025.cliente: ~3 rows (aproximadamente)
INSERT INTO `cliente` (`codCliente`, `nomeCliente`, `dataNascimento`, `telefones`, `email`, `endereco`, `bairro`, `cidade`, `cep`, `estado`, `cpf`) VALUES
	...
	(3, 'Patrick Pereira', '1980-02-15', NULL, NULL, 'filha Da Flor, 24', 'Centro', 'Alfenas', '37-135-248', 'MG', NULL),
	(4, 'Roseli Xavier', NULL, NULL, NULL, NULL, NULL, 'Machado', '37.750-000', 'MG', '023.040.984.13');
...
-- Copiando dados para a tabela lanchonete2e2025.marca: ~5 rows (aproximadamente)
INSERT INTO `marca` (`codMarca`, `nomeMarca`) VALUES
	(1, 'Fabricaçao Propria'),
	(2, 'Coca-Cola'),
	(4, 'Nestlé'),
	(5, 'Elma Chips'),
	(6, 'bauducco');

-- Copiando estrutura para procedure lanchonete2e2025.proc_HelloWorld
DROP PROCEDURE IF EXISTS `proc_HelloWorld`;
DELIMITER //
...
-- Copiando estrutura para procedure lanchonete2e2025.proc_helloWorldGrafico
DROP PROCEDURE IF EXISTS `proc_helloWorldGrafico`;
DELIMITER //
CREATE PROCEDURE `proc_helloWorldGrafico`()
BEGIN
	SELECT "Hello, World!";
END//
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE `proc_imprimeFrase`(IN frase VARCHAR(100))
BEGIN
...
INSERT INTO `produto` (`codProduto`, `nomeProduto`, `quantEstoque`, `quantMinima`, `precoCusto`, `precoVenda`, `margemLucro`, `validade`, `marca_codMarca`, `categoria_codCategoria`) VALUES
	(1, 'coxinha', NULL, NULL, NULL, 10.00, NULL, NULL, 1, 2),
	(2, 'banana', NULL, NULL, NULL, 10.00, NULL, NULL, 1, 2);
...
-- Copiando estrutura para view lanchonete2e2025.vi_cardapio
...
CREATE TABLE `vi_dadosclientes` (
	...
	`nasce` VARCHAR(1) NULL COLLATE 'utf8mb4_0900_ai_ci'
) ENGINE=MyISAM;
...
-- Copiando estrutura para view lanchonete2e2025.vi_ordenacategorias
DROP VIEW IF EXISTS `vi_ordenacategorias`;
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `vi_ordenacategorias` (
	`nomeCategoria` VARCHAR(1) NOT NULL COLLATE 'utf8mb3_general_ci'
) ENGINE=MyISAM;
...
-- Copiando estrutura para view lanchonete2e2025.vi_totalmarcas
DROP VIEW IF EXISTS `vi_totalmarcas`;
-- Criando tabela temporária para evitar erros de dependência de VIEW
CREATE TABLE `vi_totalmarcas` (
	`Total de Marcas` BIGINT NOT NULL
) ENGINE=MyISAM;

-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `vi_cardapio`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vi_cardapio` AS select `p`.`nomeProduto` AS `nomeProduto`,`m`.`nomeMarca` AS `nomeMarca`,`c`.`nomeCategoria` AS `nomeCategoria`,`p`.`precoVenda` AS `precoVenda` from ((`produto` `p` join `marca` `m` on((`p`.`codProduto` = `m`.`codMarca`))) join `categoria` `c` on((`p`.`codProduto` = `c`.`codCategoria`)))
;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vi_dadosclientes` AS select `c`.`nomeCliente` AS `nomeCliente`,`c`.`cpf` AS `cpf`,date_format(`c`.`dataNascimento`,'%d/%m/%Y') AS `nasce` from `cliente` `c` where ((`c`.`dataNascimento` is not null) or (`c`.`cpf` is not null))
;

-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `vi_idadeclientes`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vi_idadeclientes` AS select `c`.`nomeCliente` AS `nomeCliente`,date_format(`c`.`dataNascimento`,'%d/%m/%Y') AS `nasce`,floor(((to_days(curdate()) - to_days(`c`.`dataNascimento`)) / 365)) AS `idade` from `cliente` `c` where (`c`.`dataNascimento` is not null)
;
...
-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `vi_ordenacategorias`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vi_ordenacategorias` AS select `c`.`nomeCategoria` AS `nomeCategoria` from `categoria` `c` order by `c`.`nomeCategoria` desc
;
...
-- Removendo tabela temporária e criando a estrutura VIEW final
DROP TABLE IF EXISTS `vi_totalmarcas`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `vi_totalmarcas` AS select count(0) AS `Total de Marcas` from `marca`
;
